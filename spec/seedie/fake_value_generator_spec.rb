...
describe Seedie::FakeValueGenerator do
  let(:name) { 'name' }
  let(:column) { instance_double("column") }
  let(:fake_value_generator) { described_class.new(name, column) }
  ...
  describe '#generate_fake_value' do
    context 'when column type is string' do
      it 'returns a faker word' do
        ...
        expect(fake_value_generator.generate_fake_value).to eq('random_word')
      end
    end
    ...
    context 'when column type is text' do
      it 'returns a faker paragraph' do
        ...
        expect(fake_value_generator.generate_fake_value).to eq('random_paragraph')
      end
    end
    ...
    context 'when column type is integer' do
      it 'returns a faker number' do
        ...
        expect(fake_value_generator.generate_fake_value).to eq(45)
      end
    end
    ...
    context 'when column type is datetime' do
      it 'returns a faker time between now and 1 day ago' do
        ...
        expect(fake_value_generator.generate_fake_value).to eq(fake_time)
      end
    end
    ...
    context 'when column type is boolean' do
      it 'returns a faker boolean' do
        ...
        expect(fake_value_generator.generate_fake_value).to eq(true)
      end
    end
    ...
    context 'when column type is unknown' do
      it 'raises an error' do
        ...
        expect { fake_value_generator.generate_fake_value }.to raise_error("Unknown column type: unknown")
      end
    end
  end
end
